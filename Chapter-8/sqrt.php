    ...
    trigger_error(
        sprintf(
            'Decimal numbers are not allowed ' .
                'for this operation. Will use the rounded integer value [%d]',
            $input
        ),
        E_USER_WARNING
    );
...
if ($input < 0) {
    $input = abs($input);
    trigger_error(
        sprintf('A negative number is not allowed for this ' .
            'operation. Will use the absolute value [%d].',  $input),
        E_USER_WARNING
    );
}

// we print the square root of the input
