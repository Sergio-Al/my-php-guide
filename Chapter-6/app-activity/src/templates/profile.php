    ...
    <div class="col-sm-6 mb-5">
        ...
        <form action="/profile" method="post" enctype="application/x-www-form-urlencoded">
            ...
            <div class="form-label-group mb-3">
                ...
                <input type="text" name="name" id="name" class="form-control <?= isset($formErrors['name']) ? 'is-invalid' : ''; ?>" value="<?= htmlentities($_POST['name'] ?? ''); ?>">
                <?php
            ...
            <div class="form-label-group mb-3">
                ...
                <input type="text" name="email" id="email" class="form-control <?= isset($formErrors['email']) ? 'is-invalid' : ''; ?>" value="<?= htmlentities($_POST['email'] ?? ''); ?>">
                <?php
            ...
            <div class="form-label-group  mb-3">
                ...
                <textarea name="message" id="message" cols="30" rows="4" class="form-control <?= isset($formErrors['message']) ? 'is-invalid' : ''; ?>"> <?= htmlentities($_POST['message'] ?? '') ?></textarea>
                <?php if (isset($formErrors['message'])) {
                    echo sprintf('<div class="invalid-feedback">%s</div>', htmlentities($formErrors['message']));
                } ?>
            </div>
            <input type="hidden" name="csrf-token" value="<?= $formCsrfToken // this is for security purposes
                                                            ?>">
                ...
                <div class="card mb-2">
                    <div class="card-body">
                        ...
                        <h6 class="card-subtitle mb-2 text-muted">
                            <strong>Added:</strong> <?= htmlentities($item['timeAdded']) ?>
                        </h6>
                        <h6 class="card-subtitle mb-2 text-muted">
