...
class Database
{
    ...
    {
        $dsn = "mysql:host=localhost;port=3306;dbname=my_contacts;charset=utf8mb4";
        $options = [PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC];
        $this->pdo = new PDO($dsn, "yourDBUsername", "yourDBPassword", $options);
    }

    // instance method to return the same instance of Database when this method is invoked (the singleton pattern)
    ...
    public function addUser(string $username, string $password): PDOStatement
    {
        ...
        $stmt->execute([':user' => $username, ':pass' => password_hash($password, PASSWORD_BCRYPT)]);
        return $stmt;
    }
